...
Adds the mandatory security headers to every response — including
...
``Content-Security-Policy``    default-src 'self'; script-src 'self';
                               img-src 'self' https:; object-src 'none';
                               frame-ancestors 'none'

Usage
-----
This module provides a standalone middleware that explicitly sets the
required security headers.  The main application uses the ``secure``
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    """Injects the mandatory security headers on every response."""
